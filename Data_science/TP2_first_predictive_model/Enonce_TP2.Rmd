...
```{r}
mean( tit_train$gender[tit_train$survived == 1])
```
...
# Commenter la boucle ligne par ligne  
  for (i in 1:n){
    ...
    #on récupère les passagers du titanic présentant les mêmes caractéristiques (gender et age) que le passager choisi 
...
p = pred
 ...
 L = -sum(sapply(1:length(y),FUN = function(i){y[i]*log(p[i])+(1-y[i])*log(1-p[i])}))/length(y)
...
mean(survived[ !gender & class1])
cat("Probabilité de survie d'une femme en classe 3 : ")
mean(survived[ !gender & class3 ])
...
```{r}
# commenter
  table(tit_train$survived, pred.nnet )

# commenter le code 
  accuracy <- sum(tit_train$survived == (pred.nnet > 0.5 ))/nrow(tit_train)
  cat("Accuracy: ", accuracy,"\n")

# commenter le code
  pred.nnet[pred.nnet == 1] <- 0.99999999
  L <- -sum(sapply(1:length(y),FUN = function(i){y[i]*log(pred.nnet[i])+(1-y[i])*log(1-pred.nnet[i])}))/length(y)
  cat("logloss: ", L )
```
