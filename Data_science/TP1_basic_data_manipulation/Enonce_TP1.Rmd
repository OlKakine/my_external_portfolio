...
```{r}
  ...
  # comment 1
  x0 <- mvrnorm(10000, mu = c(0,0), Sigma = C)
  head(x0)
...
d2_maha <- function(x, x0, C = cov(x0)){
  ...
  m0 <- apply(x0, MARGIN = 2, FUN = mean)
  
  # comment 2
  d2 <- t(x - m0) %*% solve(C) %*% (x - m0)
...
* Pour tout vecteur contenu dans l'objet `x`, calculer le carré de la distance de mahalanobis séparant ce vecteur au centre de la classe à laquelle il appartient. Compléter le code suivant.
...
$$
et dans la classe en 2 sinon. Compléter le code suivant. 
